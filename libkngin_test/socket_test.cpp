...
client (void *_args)
{
    ...
    int _reply = 0;
    {
        ...
        if (_server_sock.read(_buf, _buf.size()) < 0)
            log_error("%s", strerror(errno));
        _buf.reset(0);
        log_info("myport %d", _buf.read_int32());
    }
    std::cin >> _reply;
    // write
    {
...
server (void *_args)
{
    ...
    if (_server_sock.bind(_server_addr) < 0)
        log_error("%s", strerror(errno));
    if (_server_sock.opts().set_reuseaddr(true).code)
        log_error("%s", strerror(errno));
    if (_server_sock.opts().set_reuseport(true).code)
        log_error("%s", strerror(errno));
    ...
    while (_ok) {
        ...
        // read
        {
            ...
            if (_client_sock.read(_buf, _buf.size()) < 0)
                log_error("%s", strerror(errno));
            if (_buf.read_int32() == 0)
                _ok = false;
            _buf.reset(0);
            log_info("read integer %d from client %d:%d",
                    _buf.read_int32(),
